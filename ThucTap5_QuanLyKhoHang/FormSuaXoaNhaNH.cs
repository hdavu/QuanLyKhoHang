...
{
    public partial class FormSuaXoaNhaNH : Form
    {
        public FormSuaXoaNhaNH()
        {
            InitializeComponent();
        }
        ...
        private void LoadData() //hien hti du lieu len cac o textbox
        {
            ...
            SqlDataAdapter sda = new SqlDataAdapter("SELECT ma, ten, diachi, sdt, masothue FROM nhanhanhang WHERE ten LIKE N'%" + txtTim.Text + "%'", con);
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            {
                con.Open();
                string sua = string.Format("delete from nhanhanhang where ma={0}", txtMa.Text);
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            if (txtDiachi.TextLength > 0 && txtMst.TextLength > 0 && txtSdt.TextLength > 0 && txtTen.TextLength > 0)
            {
                if (MessageBox.Show("Bạn muốn sửa thông tin?", "Xác nhận", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    con.Open();
                    string sua = string.Format("update nhanhanhang set ten=N'{0}', diachi =N'{1}', sdt=N'{2}' , masothue=N'{3}' WHERE ma =N'{4}'  ", txtTen.Text, txtDiachi.Text, txtSdt.Text, txtMst.Text, txtMa.Text);
                    SqlCommand cmd = new SqlCommand(sua, con);
        ...
        private void FormSuaXoaNhaNH_Load(object sender, EventArgs e)
        {
            trangthaidau();
        }
    }
